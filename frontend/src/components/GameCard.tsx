      ...
      {viewMode === 'list' ? (
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {hasStreams ? (
              ...
              <span className="text-[11px] font-semibold text-slate-400">No stream</span>
            )}
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="mb-3 flex text-xs">
            {timeLabel && (
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-medium text-slate-700">🕐 {timeLabel}</div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between border-t border-slate-100 pt-3">
            <span
              className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                hasStreams ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
              }`}
            >
              {hasStreams ? `${streamCount} stream${streamCount !== 1 ? 's' : ''}` : 'No streams'}
            </span>
